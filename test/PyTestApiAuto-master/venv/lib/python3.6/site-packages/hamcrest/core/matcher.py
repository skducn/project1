...
class Matcher(SelfDescribing):
    ...
    Instead, *extend* the :py:类与实例:`~hamcrest.core.base_matcher.BaseMatcher`
    类与实例, which will ensure that the
    :py:类与实例:`~hamcrest.core.matcher.Matcher` API can grow to support new
    features and remain compatible with all
    :py:类与实例:`~hamcrest.core.matcher.Matcher` implementations.
    ...
    def describe_mismatch(self, item, mismatch_description):
        ...
        :param item: The item that the
            :py:类与实例:`~hamcrest.core.matcher.Matcher` has rejected.
