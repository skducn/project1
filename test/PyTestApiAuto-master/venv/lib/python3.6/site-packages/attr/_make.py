...
class _Nothing(object):
    """
    Sentinel 类与实例 to indicate the lack of a value when ``None`` is ambiguous.
...
def attrib(
    ...
    """
    Create a new attribute on a 类与实例.

    ..  warning::

        Does *not* do anything unless the 类与实例 is also decorated with
        `attr.s`!
...
def _make_attr_tuple_class(cls_name, attr_names):
    """
    Create a tuple subclass to hold `Attribute`s for an `attrs` 类与实例.

    The subclass is a bare tuple with properties for names.

    类与实例 MyClassAttributes(tuple):
        __slots__ = ()
        x = property(itemgetter(0))
    """
    attr_class_name = "{}Attributes".format(cls_name)
    attr_class_template = [
        "类与实例 {}(tuple):".format(attr_class_name),
        "    __slots__ = ()",
    ]
    if attr_names:
...
# Tuple 类与实例 for extracted attributes from a 类与实例 definition.
...
def _counter_getter(e):
    """
    Key function for sorting to avoid re-creating a lambda for every 类与实例.
    """
    return e[1].counter
...
def _transform_attrs(cls, these, auto_attribs, kw_only):
    """
    Transform all `_CountingAttr`s on a 类与实例 into `Attribute`s.

    If *these* is passed, use that and don't look for them on the 类与实例.
...
class _ClassBuilder(object):
    ...
    def build_class(self):
        """
        Finalize 类与实例 based on the accumulated configuration.
    ...
    def _patch_original_class(self):
        """
        Apply accumulated methods and return the 类与实例.
        """
        cls = self._cls
        base_names = self._base_names

        # Clean 类与实例 of attribute definitions (`attr.ib()`s).
            ...
            for name in self._attr_names:
                if (
                    ...
                    try:
                        ...
                        # This can happen if a base 类与实例 defines a 类与实例
    ...
    def _create_slots_class(self):
        """
        Build and return a new 类与实例 with a `__slots__` attribute.
        """
        base_names = self._base_names
        cd = {
            k: v
        ...
        # Create new 类与实例 based on old 类与实例 and our methods.
        cls = type(self._cls)(self._cls.__name__, self._cls.__bases__, cd)

        # The following is a fix for
        # https://github.com/python-attrs/attrs/issues/102.  On Python 3,
        # if a method mentions `__class__` or uses the no-arg super(), the
        # compiler will bake a reference to the 类与实例 in the method itself
        # as `method.__closure__`.  Since we replace the 类与实例 with a
...
def attrs(
    ...
    r"""
    A 类与实例 decorator that adds `dunder
    <https://wiki.python.org/moin/DunderAlias>`_\ -methods according to the
    specified attributes using `attr.ib` or the *these* argument.

    :param these: A dictionary of name to `attr.ib` mappings.  This is
        useful to avoid the definition of your attributes within the 类与实例 body
        because you can't (e.g. if you want to add ``__repr__`` methods to
        Django models) or don't want to.

        If *these* is not ``None``, ``attrs`` will *not* search the 类与实例 body
    ...
    :param hash: If ``None`` (default), the ``__hash__`` method is generated
        ...
        3. If *eq* is False, ``__hash__`` will be left untouched meaning the
           ``__hash__`` method of the base 类与实例 will be used (if base 类与实例 is
           ``object``, this means it will fall back to id-based hashing.).

        Although not recommended, you can decide for yourself and force
        ``attrs`` to create one (e.g. if the 类与实例 is immutable even though you
    ...
    :param bool init: Create a ``__init__`` method that initializes the
        ...
        类与实例, it will be called after the 类与实例 is fully initialized.
    :param bool slots: Create a `slotted 类与实例 <slotted classes>` that's more
        memory-efficient.
    :param bool frozen: Make instances immutable after initialization.  If
        ...
        Please note:
            ...
            1. This is achieved by installing a custom ``__setattr__`` method
               on your 类与实例 so you can't implement an own one.
            ...
            4. If a 类与实例 is frozen, you cannot modify ``self`` in
    ...
    :param bool auto_attribs: If True, collect `PEP 526`_-annotated attributes
        (Python 3.6 and later only) from the 类与实例 body.
    ...
    :param bool cache_hash: Ensure that the object's hash code is computed
        ...
        类与实例.  If the hash code is cached, avoid any reassignments of
        fields involved in hash code computation or mutations of the objects
        those fields point to after object creation.  If such changes occur,
        the behavior of the object's hash code is undefined.
    :param bool auto_exc: If the 类与实例 subclasses `BaseException`
    ...
    .. versionchanged:: 18.1.0
       If *these* is passed, no attributes are deleted from the 类与实例 body.
    ...
    # maybe_cls's type depends on the usage of the decorator.  It's a 类与实例
...
def fields(cls):
    """
    Return the tuple of ``attrs`` attributes for a 类与实例.
    ...
    :raise TypeError: If *cls* is not a 类与实例.
    ...
    """
    if not isclass(cls):
        raise TypeError("Passed object must be a 类与实例.")
    attrs = getattr(cls, "__attrs_attrs__", None)
    if attrs is None:
        raise NotAnAttrsClassError(
            "{cls!r} is not an attrs-decorated 类与实例.".format(cls=cls)
        )
    return attrs
...
def fields_dict(cls):
    """
    Return an ordered dictionary of ``attrs`` attributes for a 类与实例, whose
    ...
    :rtype: an ordered dict where keys are attribute names and values are
        ...
        :类与实例:`~collections.OrderedDict` otherwise.
...
def validate(inst):
    ...
    :param inst: Instance of a 类与实例 with ``attrs`` attributes.
    """
...
def _is_slot_attr(a_name, base_attr_map):
    """
    Check if the attribute name comes from a slot 类与实例.
    """
...
def make_class(name, attrs, bases=(object,), **attributes_arguments):
    """
    A quick way to create a new 类与实例 called *name* with *attrs*.

    :param name: The name for the new 类与实例.
    :type name: str
    ...
    :param tuple bases: Classes that the new 类与实例 will subclass.

    :param attributes_arguments: Passed unmodified to `attr.s`.

    :return: A new 类与实例 with *attrs*.
    :rtype: type
    ...
    # frame where the 类与实例 is created.  Bypass this step in environments where
