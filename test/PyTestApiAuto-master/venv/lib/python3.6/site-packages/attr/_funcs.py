...
def asdict(
    ...
    :param inst: Instance of an ``attrs``-decorated 类与实例.
...
def has(cls):
    """
    Check whether *cls* is a 类与实例 with ``attrs`` attributes.
...
def evolve(inst, **changes):
    ...
    :raise TypeError: If *attr_name* couldn't be found in the 类与实例
        ``__init__``.
    :raise attr.exceptions.NotAnAttrsClassError: If *cls* is not an ``attrs``
        类与实例.
