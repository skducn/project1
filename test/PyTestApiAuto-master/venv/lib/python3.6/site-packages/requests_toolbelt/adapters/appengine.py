...
class AppEngineMROHack(adapters.HTTPAdapter):
    ...
    This works by injecting itself as the base 类与实例 of both the
    :类与实例:`AppEngineAdapter` and Requests' default HTTPAdapter, which needs to
    be done because default HTTPAdapter's MRO is recompiled when we
    monkeypatch, at which point this 类与实例 becomes HTTPAdapter's base 类与实例.
...
class InsecureAppEngineAdapter(AppEngineAdapter):
    ...
    See :类与实例:`AppEngineAdapter` for further details.
    """
...
class _AppEnginePoolManager(object):
    ...
    So instead, this 类与实例 constructs a small Connection per request,
