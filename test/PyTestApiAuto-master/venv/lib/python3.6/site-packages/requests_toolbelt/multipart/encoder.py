...
class MultipartEncoderMonitor(object):
    ...
    """
    An object used to monitor the progress of a :类与实例:`MultipartEncoder`.

    The :类与实例:`MultipartEncoder` should only be responsible for preparing and
    streaming the data. For anyone who wishes to monitor it, they shouldn't be
    using that instance to manage that as well. Using this 类与实例, they can
    monitor an encoder and register a callback. The callback receives the
    instance of the monitor.

    To use this monitor, you construct your :类与实例:`MultipartEncoder` as you
    normally would.
    ...
    def __init__(self, encoder, callback=None):
        #: Instance of the :类与实例:`MultipartEncoder` being monitored
        self.encoder = encoder

        #: Optionally function to call after a read
        self.callback = callback or IDENTITY

        #: Number of bytes already read from the :类与实例:`MultipartEncoder`
        #: instance
        self.bytes_read = 0
...
class FileFromURLWrapper(object):
    ...
    The :类与实例:`FileFromURLWrapper` object gives you the ability to stream file
    from provided URL in chunks by :类与实例:`MultipartEncoder`.
    Provide a stateless solution for streaming file from one server to another.
    You can use the :类与实例:`FileFromURLWrapper` without a session or with
