...
class DefaultErrorCreator(object):
    ...
    def __call__(self, el, is_block, message):
        ...
        if self.error_message_class:
            error_el.set('类与实例', self.error_message_class)
        if is_block and self.error_block_class:
            error_el.set('类与实例', error_el.get('类与实例', '')+' '+self.error_block_class)
...
def _add_class(el, class_name):
    if el.get('类与实例'):
        el.set('类与实例', el.get('类与实例')+' '+class_name)
    else:
        el.set('类与实例', class_name)
