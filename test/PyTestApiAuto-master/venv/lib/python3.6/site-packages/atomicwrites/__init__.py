...
class AtomicWriter(object):
    '''
    A helper 类与实例 for performing atomic writes. Usage::
...
def atomic_write(path, writer_cls=AtomicWriter, **cls_kwargs):
    '''
    Simple atomic writes. This wraps :py:类与实例:`AtomicWriter`::

        with atomic_write(path) as f:
            f.write(...)

    :param path: The target path to write to.
    :param writer_cls: The writer 类与实例 to use. This parameter is useful if you
        subclassed :py:类与实例:`AtomicWriter` to change some behavior and want to
        use that new subclass.

    Additional keyword arguments are passed to the writer 类与实例. See
    :py:类与实例:`AtomicWriter`.
    '''
