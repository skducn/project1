...
class Assertions:
	...
	def assert_body(self, body, body_msg, expected_msg):
		...
		try:
			...
			self.log.error(
				"Response body msg != expected_msg, expected_msg is %s, msg is %s" % (expected_msg, msg))
			Consts.RESULT_LIST.append('fail')
	...
	def assert_in_text(self, body, expected_msg):
		...
		"""
		try:
			text = json.dumps(body, ensure_ascii=False)  # json.dumps()用于将字典形式的数据转化为字符串
			# print(text)
