...
# Author     : John
# Date       : 2020-4-28
# Description: Python下ORM的一个设计举例
# https://blog.csdn.net/GVFDBDF/article/details/49254701?depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromBaidu-25&utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromBaidu-25
#***************************************************************
...
预备知识：Python协程和异步IO(await的使用)、SQL数据库操作、元类、面向对象知识、Python语法
...
async def create_pool(loop, **kw):
    ...
    __pool = await aiomysql.create_pool(host=kw.get('host', '192.168.0.195'),user=kw['root'],password=kw['Zy123456'],db=kw['bitest'],port=kw.get('port', 3306),charset=kw.get('charset', 'utf8'),autocommit=kw.get('autocommit', True),minsize=kw.get('minisize', 1),loop=loop)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def find(cls, primarykey):
        ...
        rs = await select('%s where `%s`=?' % (cls.__select__, cls__primary_key__), [primarykey], 1)
        if len(rs) == 0:
            return None
        return cls(**rs[0])
...
if __name__ == '__main__':
    class Bi_anaesthesia_dept(Model):
        # 定义类的属性到列的映射：
    ...
    u = Bi_anaesthesia_dept(id=12345)
    print(u)
    # 保存到数据库：
    u.save()
    print(u)  