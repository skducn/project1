...
d_title = {1: ['日期', '代码', '名称', '涨跌幅', '主力净流入(亿)', '超大单净占比(%)', '大单净占比(%)', '中单净占比(%)', '小单净占比(%)']}

s_path = "/Users/linghuchong/Downloads/51/Python/stock/BK4"
...
def main(s_group):
    ...
    for i in range(i_rowCount):
        ...
        if "BK" in s_code:
            ...
            s_file = s_path + "/" + s_code + "_" + all_names[i] + ".xlsx"
            ...
            try:
                ...
                if first_cell_value is None or first_cell_value != '日期':
                    # 如果标题行不存在或不匹配，则写入标题
            ...
            if s_currDate not in l_1:
                Openpyxl_PO.appendRows(l_all, "Sheet1")
            else:
                # 如果存在日期，则更新
                l_ = Openpyxl_PO.getOneCol(1, "Sheet1")
                # print(l_.index(s_currDate))
                a = l_.index(s_currDate) + 1
                Openpyxl_PO.setRows({a: l_all[0]}, "Sheet1")
            l_all = []
