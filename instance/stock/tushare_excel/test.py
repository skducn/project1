...
if not df.empty:
    ...
    volume = df['vol'].values[0]
    print(f"股票{ts_code}在{trade_date}的开盘价是：{open_price}")
    print(f"股票{ts_code}在{trade_date}的收盘价是：{close_price}")
    print(f"股票{ts_code}在{trade_date}的成交量是：{volume}")
else:
    print(f"未获取到股票{ts_code}在{trade_date}的数据")
