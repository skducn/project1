...
class DocSearchAgent:
    ...
    def search(self, keywords_input, search_mode='模糊'):
        ...
        result_count = 0
        results = []
        results.append(f"========== 检索结果（模式：{search_mode}，关键字：{keywords_input}）==========")
            ...
            for pos, content in parsed_data:
                if pattern.search(content):
                    if not file_matched:
                        ...
                        file_matched = True
                    result_count += 1
                    # 高亮关键字（移除ANSI颜色代码，因为GUI中不需要）
                    content_highlight = self._remove_highlight(content)
                    results.append(f"【定位信息】：{pos}")
                    results.append(f"【匹配内容】：{content_highlight}\n")

        if result_count == 0:
            results.append("❌ 未找到匹配内容")
        else:
            results.append(f"✅ 检索完成，共找到{result_count}条匹配结果")

        return "\n".join(results)

    def _remove_highlight(self, text):
        """移除高亮标记（GUI不需要ANSI颜色代码）"""
        import re
