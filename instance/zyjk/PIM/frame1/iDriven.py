...
class HTTP:
    ...
    def post(self, interName, param):
        ...
        if param == '':
            ...
            result = self.session.post(path, headers=self.headers, data=param)
            # result = requests.post(path, headers=self.headers, data=param)
            # print(path)
            # print(param)
            if "token" in result.text:
                self.jsonres = json.loads(result.text)
                self.session.headers['token'] = self.jsonres['extra']['token']
    ...
    def postget(self, interName, param):
        ...
        if param == '':
            ...
            result = self.session.post(path, headers=self.headers)
        print(self.session.headers)
        print(result.text)
        res = result.text
        try:
    ...
    def get(self, interName, param):
        ...
        if param == '':
            ...
            # result = requests.get(path, headers=self.headers)
            result = self.session.get(path, headers=self.headers)
        print(self.session.headers)
        print(result.text)
        res = result.text
        try:
