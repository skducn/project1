...
class BmiAgePO():
    ...
    def generate_sample(self, bmi_conditions, age_conditions, satisfy_bmi, satisfy_age, use_boundary=False):
        """
        生成一个符合指定条件组合的样本，并支持边界值或随机值
        """
        ...
        if satisfy_bmi:
            if use_boundary:
                # 使用最小合法BMI值
                bmi = self.get_min_valid_bmi(bmi_conditions)
            else:
                bmi = self.generate_valid_bmi(bmi_conditions)
        else:
            bmi = self.generate_invalid_bmi(bmi_conditions)
        ...
        if satisfy_age:
            if use_boundary:
                # 使用最小合法年龄值
                age = self.get_min_valid_age(age_conditions)
            else:
                age = self.generate_valid_age(age_conditions)
        else:
            age = self.generate_invalid_age(age_conditions)

        return {'BMI': round(bmi, 1), '年龄': round(age, 1)}

    def get_min_valid_bmi(self, conditions):
        """获取满足所有BMI条件的最小合法值"""
        min_val = 24.0  # 默认最小合法值
        for cond in conditions:
            match = re.match(r'BMI([>+=]+)(\d+)', cond)
            if not match:
                continue
            op, val = match.groups()
            val = float(val)
            if op == '>=':
                ...
                min_val = max(min_val, val + 0.1)
        return min_val

    def get_min_valid_age(self, conditions):
        """获取满足所有年龄条件的最小合法值"""
        min_val = 18.0  # 默认最小合法值
        for cond in conditions:
            match = re.match(r'年龄([>+=]+)(\d+)', cond)
            if not match:
                continue
            op, val = match.groups()
            val = float(val)
    ...
    def generate_valid_age(self, conditions):
        ...
        for condition in conditions:
            ...
            if operator == '>':
                age_min = max(age_min, value + 0.1)
            elif operator == '>=':
                age_min = max(age_min, value)
            elif operator == '<':
                age_max = min(age_max, value - 0.1)
            elif operator == '<=':
                age_max = min(age_max, value)
...
if __name__ == "__main__":
    ...
    try:
        ...
        cases = BmiAge_PO.generate_all_cases(conditions, num_samples=2)
        # d_cases = po.generate_all_cases(conditions, num_samples=2)
        print(cases)
