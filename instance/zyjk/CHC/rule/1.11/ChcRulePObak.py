...
class ChcRulePO():
    ...
    def run11(self, dbId):
        ...
        # 获取表字段值
        l_d_ = Sqlserver_PO.select("select * from %s where id=%s" % (self.dbTable, self.dbId))

        try:
            self.rule = l_d_[0]['rule']
        except:
            Color_PO.outColor([{"31": "error, rule不存在！"}])
            sys.exit(0)

        self.case = l_d_[0]['case']

        if l_d_[0]['ruleParam'] != None:
            if l_d_[0]['ruleParam'] == '':
                self.ruleParam = {}
            else:
                try:
                    self.ruleParam = dict(eval(l_d_[0]['ruleParam']))
                except:
                    Color_PO.outColor([{"31": "error, ruleParam不是字典形式字符串或不为空！"}])
                    sys.exit(0)
        else:
            self.ruleParam = {}

        self.ruleCode = l_d_[0]['ruleCode']

        if 'diseaseRuleCode' in l_d_[0].keys():
            self.diseaseRuleCode = l_d_[0]['diseaseRuleCode']
        else:
            self.diseaseRuleCode = ""

        if 'diseaseCodeDesc' in l_d_[0].keys():
            self.diseaseCodeDesc = l_d_[0]['diseaseCodeDesc']
        else:
            self.diseaseCodeDesc = ""

        if 'assessCodeDesc' in l_d_[0].keys():
            self.assessCodeDesc = l_d_[0]['assessCodeDesc']
        else:
            self.assessCodeDesc = ""

        if 'assessRuleCode' in l_d_[0].keys():
            self.assessRuleCode = l_d_[0]['assessRuleCode']
        else:
            self.assessRuleCode = ""

        self.tester = l_d_[0]['tester']
    ...
    def testRules(self):
        ...
        # print(self.tmp_db)
        if Configparser_PO.SWITCH("SQL") == "on":
            Color_PO.outColor([{"35": self.tmp_db}])
        Sqlserver_PO.crtTable(self.tmp_db, '''id INT IDENTITY(1,1) PRIMARY KEY, key1 VARCHAR(500), value1 VARCHAR(500)''')
            ...
            if l_d_ != None:
                if isinstance(l_d_, list) and l_d_ != []:
                    if isinstance(l_d_[0], dict):
                        ...
                        # 打印返回值
                        if Configparser_PO.SWITCH("SQL") == "on":
                            Color_PO.outColor([{"31": l_d_[0]}])  # {'GUID': '65209815'}
    ...
    def testRule11(self):
        ...
        for i in range(len(self.sql)):
            ...
            # s3
            if '{DIAGNOSIS_CODE3}' in self.sql[i]:
                self.sql[i] = str(self.sql[i]).replace('{DIAGNOSIS_CODE3}', str(self.prefixICD))
