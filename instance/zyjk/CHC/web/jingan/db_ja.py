...
# print(d_hospital)  # {'0000001': '静安精神病院', '42508153900': '静安区社区健康管理中心',。。。

# 获取当前登录家庭医生的工号
l_ = Sqlserver_PO2.select("select THIRD_NO,ORG_CODE from SYS_USER where user_name='cs'")
varTHIRD_NO = l_[0]['THIRD_NO']
# print(varTHIRD_NO)  # 101

# 获取当前登录家庭医生的机构号
ORG_CODE = l_[0]['ORG_CODE']
# print(ORG_CODE)  # 42508153900
...
def insert(d_param):
    ...
    Sqlserver_PO.execute("insert into QYYH(CZRYBM, CZRYXM, JMXM, SJHM, SFZH, JJDZ, SFJD, SIGNORGID, ARCHIVEUNITCODE, "
                         ...
                         "ASSISTANT_DOC_NAME, HEALTH_MANAGER_ID, HEALTH_MANAGER_NAME, ASSISTANT_DOC_PHONE, HEALTH_MANAGER_PHONE, "
                         "REPORT_STATUS, ASSESS_ORDER, STOP_ASSESS_STATUS, STOP_ASSESS_DOC_ID, STOP_ASSESS_DOC_NAME, STOP_ASSESS_DATA) "
                         "values "
                         "('%s','令狐冲','%s','%s','%s','%s','','',"
                         "'%s','%s','310118000000','青浦区','0000001','静安精神病院','','','','','','','','','','','',"
                         "%s,'%s',%s,'%s','%s','%s','%s','%s',1,'家医助理姓名',2,'健康管理师姓名','%s','%s',%s,2,0,-1,'章三','2029-10-10')"
                         % (varTHIRD_NO, varJMXM, Fake_PO.genPhone_number('Zh_CN',1), varIdCard,
                            ...
                            Fake_PO.genPhone_number('Zh_CN', 1), d_param["REPORT_STATUS"]))
    Sqlserver_PO.execute('set identity_insert QYYH off')
    print("QYYH => " + varJMXM + ", " + varIdCard)

insert({"ARCHIVEUNITCODE":ORG_CODE, "ARCHIVEUNITNAME":d_hospital[ORG_CODE],"SIGNSTATUS":1,"SIGNDATE":"2024-01-01",
        "CATEGORY_CODE":'2',"CATEGORY_NAME":d_category[2],"LAST_SERVICE_DATE":"2024-03-03","REPORT_STATUS":1})
