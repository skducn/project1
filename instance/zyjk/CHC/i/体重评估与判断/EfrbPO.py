...
class EfrbPO():
    ...
    def EFRB_result(self, s_):
        # 执行错误
        l_d_row = Sqlserver_PO_CHC.select("select id, category, conditions, ER_code from %s where result='%s'" % (self.tableEFRB, s_))
        # self._EFRB_one(l_d_row, d_param)
        for row in l_d_row:
            d_param = {
                'ER_code': row['ER_code'],
            ...
            if len(d_cases['notSatisfied']) == 1:
                ...
                for i in range(len(d_cases['notSatisfied'])):
                    ...
                    if d_tmp['result'] == 1:
                        ...
                        d_1.update(d_tmp)
                        s_tmp = str(d_1).replace("\\\\", "\\")
                        s_print = f"{i + 1}/{len(d_cases['notSatisfied'])}, " + s_tmp
                        l_error_log.append(s_print.replace("'", "''"))
                        Color_PO.outColor([{"31": s_print}])
                        Log_PO.logger.info(s_print)
                        l_count.append(0)
    ...
    def updateDB(self, d_param):
        ...
        if 0 not in d_param['l_count']:
            ...
            Sqlserver_PO_CHC.execute(
                "update %s set result = 'ok', updateDate = GETDATE(), totalCase=%s, log=Null where id = %s" %
