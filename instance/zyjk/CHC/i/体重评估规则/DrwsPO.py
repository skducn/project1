...
class DrwsPO():
    ...
    def __init__(self):
        self.tableDRWS = Configparser_PO.DB("tableDRWS")
    ...
    def DRWS(self, varTestID="all"):
        ...
        # 获取表每行测试数据
        l_d_row = Sqlserver_PO_CHC.select("select [id], [conditions] from [%s]" % (self.tableDRWS))
        # l_d_row = Sqlserver_PO_CHC.select("select id, conditions from %s" % (self.tableDRWS))
        # print("l_d_row =>", l_d_row)  # [{'ID': 1, 'conditions': 'BMI＜18.5'}, ...

        # 添加检查确保查询返回了结果
        if not l_d_row or len(l_d_row) == 0:
            raise Exception(f"表 {self.tableDRWS} 中没有找到测试数据")
        ...
        for i, index in enumerate(l_d_row):
            ...
            # todo 4 DRWS 单个条件（BMI＜18.5）
                ...
                l_split_conditions = Bmi_PO.splitMode(conditions)
                l_conditions.extend(l_split_conditions)
    ...
    def _DRWS_result(self, count, totalCase, id):
        d_result = {}
        if 0 not in count:
            d_result['id'] = id
            ...
            Color_PO.outColor([{"32": s}])
            Log_PO.logger.info(s)
            # Sqlserver_PO_CHC.execute("update %s set result = 'ok', updateDate = GETDATE(), totalCase=%s where id = %s" % (self.tableDRWS, totalCase, id))
            Sqlserver_PO_CHC.execute(
                "update [%s] set [result] = 'ok', [updateDate] = GETDATE(), [totalCase]=%s where [id] = %s" % (
                self.tableDRWS, totalCase, id))
        else:
            d_result['id'] = id
            ...
            Sqlserver_PO_CHC.execute(
                ...
                self.tableDRWS, totalCase, id))
            # Sqlserver_PO_CHC.execute("update %s set result = 'error', updateDate = GETDATE(), totalCase=%s where id = %s" % (self.tableDRWS, totalCase, id))
    ...
    def DRWS_case_or(self, d_cases, id, l_conditions, Numerator, Denominator):
        ...
        if len(d_cases['satisfied']) == 1:
            ...
            if d_tmp['result'] == 1:
                ...
                s_tmp = s_tmp.replace("'", "''")
                s_tmp = s_tmp.replace("\\\\", "\\")
                Sqlserver_PO_CHC.execute("UPDATE %s SET log = '%s' where id=%s" % (self.tableDRWS, s_tmp, d_tmp['id']))
            ...
            for i in range(len(d_cases['satisfied'])):
                ...
                if d_tmp['result'] == 1:
                    ...
                    Log_PO.logger.info("判定居民体重状态DRWS => {'表': '" + self.tableDRWS + "', 'id': " + str(id) + "}")
                    d_1['No.'] = str(Numerator) + "(" + str(i + 1) + ")/" + str(Denominator)
                    d_1['正向'] = 'error'
                    d_1['条件'] = l_conditions
                    d_1['验证'] = d_cases['satisfied'][i]
                    ...
                    s_tmp = s_tmp.replace("\\\\", "\\")
                    print(619,d_tmp)
                    Sqlserver_PO_CHC.execute("UPDATE %s SET log = '%s' where id=%s" % (self.tableDRWS, s_tmp, id))
                    # Sqlserver_PO_CHC.execute("UPDATE %s SET log = '%s' where id=%s" % (self.tableDRWS, s_tmp, d_tmp['id']))
                caseTotal = caseTotal + 1
                # Log_PO.logger.info("---------------------------------------------------------------------")
            ...
            if Configparser_PO.SWITCH("testNegative") == "on":
                for i in range(len(d_cases['notSatisfied'])):
                    ...
                    if d_tmp['result'] == 1:
                        # 反向如果命中就错，并且终止循环
                        Log_PO.logger.info("判定居民体重状态DRWS => {'表': '" + self.tableDRWS + "', 'id_pk': " + str(id) + "}")
    ...
    def _DRWS_run(self, d_cases_satisfied, id):
        ...
        # # 参数
        # l_d_row = Sqlserver_PO_CHC.select(
        #     "select category, categoryCode, weightStatus, weightStatusCode from %s where id= %s" % (self.tableDRWS, id))

        # print(l_d_row)  # [{'category_nvc': '普通人群', 'categoryCode_vc': '3', 'weightStatus_nvc': '体重偏低', 'weightStatusCode_vc': '1'}]
        d_tmp['人群分类'] = l_d_row[0]['category']  # 普通人群
        d_tmp['人群分类编码'] = l_d_row[0]['categoryCode']  # 3
        d_tmp['体重状态'] = l_d_row[0]['weightStatus']  # 1
        d_tmp['体重状态编码'] = l_d_row[0]['weightStatusCode']  # 1
