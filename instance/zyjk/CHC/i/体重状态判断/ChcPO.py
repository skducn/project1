...
class ChcPO():
    ...
    def main_one(self, id):
        ...
        l_d_ = Sqlserver_PO.select("select category, categoryCode, weightStatus, weightStatusCode, f_rule from %s where id=%s" % (self.table, id))
        ...
        if l_log == []:
            ...
            Sqlserver_PO.execute("update %s set result = '%s', updateDate = GETDATE(), log='%s' where id = %s"
                                 % (self.table, 'error', escaped_log, id))
    ...
    def main(self, d_=None):
        # Chc_PO.main({"id": 1})
        # Chc_PO.main({"category": '学生'})
        # Chc_PO.main({"weightStatus": '体重偏低'})
        # Chc_PO.main({"category": '学生', "weightStatus": '体重偏低'})
        ...
        if isinstance(d_, dict):
            if 'id' in d_:
                ...
                if isinstance(d_['id'], list):
                    ...
                    # 执行一条(id)，如：{'id': 56}
                    # 判断id是否溢出
                    l_d_row = Sqlserver_PO.select("select * from %s" % (self.table))
                ...
                # 执行一条
                # Chc_PO.main({"category": '学生', "weightStatus": '体重偏低'})
                l_d_ = Sqlserver_PO.select("select id from %s where category='%s' and weightStatus='%s'" % (self.table, d_['category'], d_['weightStatus']))
                self.main_one(l_d_[0]['id'])
            else:
                # 执行多条
                if 'category' in d_:
                    # Chc_PO.main({"category": '学生'})  # 执行所有学生
                    l_d_ = Sqlserver_PO.select("select id from %s where category='%s'" % (self.table, d_['category']))
                    ...
                    # Chc_PO.main({"weightStatus": '体重偏低'}) # 执行所有体重偏低
                    l_d_ = Sqlserver_PO.select("select id from %s where weightStatus='%s'" % (self.table, d_['weightStatus']))
                    l_id = [item['id'] for item in l_d_]
                    # print(l_id)  # 输出：[1, 2, 3, 4]
                    self.main_multiple(l_id)
                else:
            ...
            # 执行所有，如：{'id': 'all'}
            l_d_ = Sqlserver_PO.select("select id from %s " % (self.table))
            l_id = [item['id'] for item in l_d_]
            # print(l_id)  # 输出：[1, 2, 3, 4]
            self.main_multiple(l_id)
    ...
    def excel2db(self):
        ...
        try:
            ...
            # 1, db中删除已有的表
            Sqlserver_PO.execute("drop table if exists " + self.table)

            # 读取 Excel 文件
            df = pd.read_excel(self.case, sheet_name=self.sheet)
            ...
            # 2, excel导入db
            Sqlserver_PO.df2db(df, self.table)

            # 3, 设置表注释
            Sqlserver_PO.setTableComment(self.table, '体重管理_体重状态判断_自动化测试')

            # 4， f_rule，替换换行符为空格
            Sqlserver_PO.execute("UPDATE %s SET f_rule = REPLACE(REPLACE(f_rule, CHAR(10), ' '), CHAR(13), ' ');" % (self.table))
            ...
            for field_name, field_type, comment in field_definitions:
                Sqlserver_PO.setFieldTypeComment(self.table, field_name, field_type, comment)

            Sqlserver_PO.execute("ALTER TABLE %s ALTER COLUMN updateDate DATE;" % (self.table))

            # 6, 设置自增主键（最后）
            Sqlserver_PO.setIdentityPrimaryKey(self.table, "id")
