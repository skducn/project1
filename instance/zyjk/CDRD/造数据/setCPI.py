...
def aes_encrypt(plaintext, key):
    ...
    if isinstance(plaintext, str):
        plaintext_bytes = plaintext.encode('GBK')
    else:
...
for i in range(5):
    ...
    random_name_encrypted = aes_encrypt(random_name, aes_key)

    # 随机地址
    random_address = random.choice(["南京路100号", "中山路11号", "复兴路44号", "东方路10号", "七浦路89号", "鲁班路54号"])
    random_address_encrypted = aes_encrypt(random_address, aes_key)

    # 手机
    random_phone = Data_PO.getPhone()
    random_phone_encrypted = aes_encrypt(random_phone, aes_key)

    # 构建SQL语句 - 使用参数化查询直接插入二进制数据
    ...
    # 参数列表 - 所有字符串参数都明确编码为字节，二进制数据保持原样
    params = (
        random_name.encode('GBK'),  # PATIENT_NAME
        d_gender[sex],  # PATIENT_SEX_KEY
        sex.encode('GBK'),  # PATIENT_SEX_VALUE
        birthday.encode('GBK'),  # PATIENT_BIRTH_DATE
        age,  # PATIENT_AGE
        random_province.get('PROVINCECODE', '000000').encode('GBK'),  # PATIENT_BIRTH_ADDRESS_PROVINCE_KEY
        random_province.get('NAME', '未知省份').encode('GBK'),  # PATIENT_BIRTH_ADDRESS_PROVINCE
        random_city.get('CITYCODE', '000000').encode('GBK'),  # PATIENT_BIRTH_ADDRESS_CITY_KEY
        random_city.get('NAME', '未知城市').encode('GBK'),  # PATIENT_BIRTH_ADDRESS_CITY
        random_city.get('CODE', '000000').encode('GBK'),  # PATIENT_BIRTH_ADDRESS_COUNTRY_KEY
        random_city.get('NAME', '未知城市').encode('GBK'),  # PATIENT_BIRTH_ADDRESS_COUNTRY
        '中国'.encode('GBK'),  # PATIENT_COUNTRY
        random_province.get('PROVINCECODE', '000000').encode('GBK'),  # PATIENT_NATIVE_PROVINCE_KEY
        random_province.get('NAME', '未知省份').encode('GBK'),  # PATIENT_NATIVE_PROVINCE
        random_city.get('CITYCODE', '000000').encode('GBK'),  # PATIENT_NATIVE_CITY_KEY
        random_city.get('NAME', '未知城市').encode('GBK'),  # PATIENT_NATIVE_CITY
        random_nation.get('DICT_VALUE', '1').encode('GBK'),  # PATIENT_NATION_KEY
        random_nation.get('DICT_LABEL', '中国').encode('GBK'),  # PATIENT_NATION_VALUE
        random_job.encode('GBK'),  # PATIENT_PROFESSION
        random_marriage.get('DICT_VALUE', '1').encode('GBK'),  # PATIENT_MARRIAGE_KEY
        random_marriage.get('DICT_LABEL', '未婚').encode('GBK'),  # PATIENT_MARRIAGE_VALUE
        random_id_type.get('DICT_VALUE', '1').encode('GBK'),  # PATIENT_ID_TYPE_KEY
        random_id_type.get('DICT_LABEL', '身份证').encode('GBK'),  # PATIENT_ID_TYPE_VALUE
        random_address.encode('GBK'),  # PATIENT_ACCOUNT_ADDRESS
        random_relation.encode('GBK'),  # PATIENT_CONTACT_RELATION
        Time_PO.getDateTimeByPeriod(0).encode('GBK'),  # PATIENT_UPDATE_TIME
        '1'.encode('GBK'),  # PATIENT_DATA_SOURCE_KEY
        ''.encode('GBK'),  # PATIENT_SOURCE_ID
        random_phone_encrypted,  # PATIENT_PHONE_NUM (二进制数据流)
        random_address_encrypted,  # PATIENT_HOME_ADDRESS (二进制数据流)
        idcard.encode('GBK'),  # PATIENT_ID_NUM
