...
CREATE OR ALTER PROCEDURE s_sys_menu
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @isExist = menu_id FROM sys_menu WHERE menu_name = @menuName and menu_type = @menuType;
        ...
        BEGIN
            -- 统计顺序
            SELECT @isCount = count(*) FROM sys_menu WHERE menu_type = @menuType;
            set @isCount = @isCount + 1;

            -- 插入目录
            INSERT INTO sys_menu (menu_name,order_num,path,component,query,route_name,is_frame,is_cache,menu_type,status,visible,perms,icon,create_by,create_time,update_by,update_time,remark)
            VALUES (
                    @menuName, -- 菜单名称
                    @isCount, -- 显示顺序
                    '/test/', -- 路由地址
        ...
        BEGIN
            ...
            -- 获取父级ID
            SELECT @parent_id = menu_id FROM sys_menu WHERE menu_name = @menuParentName and menu_type = 'M';

            IF @parent_id IS NOT NULL
            BEGIN
                -- 插入目录
                INSERT INTO sys_menu (menu_name,parent_id,order_num,path,component,query,route_name,is_frame,is_cache,menu_type,status,visible,perms,icon,create_by,create_time,update_by,update_time,remark)
                VALUES (
                        @menuName, -- 菜单名称
                        @parent_id, -- 父菜单ID
                        @isCount, -- 显示顺序
        ...
        BEGIN
            ...
            -- 获取父级ID
            SELECT @parent_id = menu_id FROM sys_menu WHERE menu_name = @menuParentName and menu_type = 'C';
