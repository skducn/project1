...
# 生成默认数据
test_data = Fake_PO.genTest(['genCompany'], 1)
...
def step_screenshot(page, step_name):
    """步骤化截图函数"""
    if Configparser_PO.用户管理("screenshots") == "on":
...
def run_playwright():
    ...
    with sync_playwright() as p:
        ...
        if Configparser_PO.用户管理("videos") == "on":
            # 开启录屏
        ...
        page.get_by_placeholder("请输入登录账号").fill("016")
        page.get_by_placeholder("请输入密码").click()
        page.get_by_placeholder("请输入密码").fill("Aa@123456")
        page.get_by_role("button", name="登 录").click()

        # 编辑用户
        page.locator("div").filter(has_text=re.compile(r"^系统管理$")).click()
        page.get_by_role("link", name="用户管理").click()
        page.get_by_role("row", name="王玉华 608466 148027 眼科(0023)").get_by_role("button").first.click()
        page.get_by_label("备注").click()
        page.get_by_label("备注").fill(test_data[0][0])
        step_screenshot(page, "编辑用户")  # 截图

        # 保存
        if Configparser_PO.用户管理("isSave") == "on":
