...
# Description: 电子健康档案，ITF与DC库表字段数据比对。（徐汇）
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
Sqlserver_PO_itf = SqlServerPO("192.168.60.12", "sa", "1qaz@wsx", "EHRITF","GBK")
Sqlserver_PO_dc = SqlServerPO("192.168.60.12", "sa", "1qaz@wsx", "EHRDC","GBK")

# 测试次数（随机身份证）
QTY = 1

# 输出比对字段数量
print((" 测试" + str(Openpyxl_PO.l_getTotalRowCol()[0]-1) + "个字段 ").center(50, "-"))
...
for i in range(QTY):
        ...
        # print(r_ITF_TB_EXAMINATION_INFO)
        ...
        # print(r_tb_dc_examination_info)
