...
Web_PO.opnLabel(d_menu_basicPHS['孕产妇档案'])
Web_PO.swhLabel(1)



# todo 1 查询
Gw_PO.phs_maternalRecord_ycffiles_query({"身份证号": "370685198705183027"})
# Gw_PO.phs_maternalRecord_ycffiles_query({"管理机构": ["招远市卫健局"], '是否仅查询机构': '是', "档案状态": "在档", "姓名": "胡成", "身份证号": "370685198705183027", '上次完成 检查类型': '第2次产前检查'})


# # todo 2 健康检查记录
Gw_PO.phs_maternalRecord_ycffiles_operation({'operate': '健康检查记录', 'option': {"身份证号": "370685198705183027"}})

# 第一次产前随访服务记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'operate': '编辑', 'title': '第一次产前随访服务记录表', 'data': {
...
# 第2～5次产前随访服务记录表
Gw_PO.phs_maternalRecord_ycffiles_operation({'operate': '编辑', 'title': '第2～5次产前随访服务记录表', 'data': {
    '第2次': {'日期': [2024, 1, 1], '本次服务类别': {'随访': ''}, '随访方式': '电话', '孕周': ["1","3"],
        '产前检查机构名称': '1', '主诉': '2', '体重（kg）':'100','宫底高度(cm)':'144','腹围(cm)':'88',
        '胎位':['右枕前（ROA）', '左枕后（LOP）', '左颏前（LMA）'], '胎心率(次/分钟)': ['45', '66', '88'],
        '血压（mmHg）':['111','99'],'血红蛋白（g/L）':'234','尿蛋白': '+','其他辅助检查':'678',
        '免费血清学产前筛查': {'是': ['爱德华氏综合征', '唐氏综合征']},
        '随访结果分类': {'异常':'13123'}, '指导': ['营养', {'其他': '4234'}],
        '转诊': ['有', {'原因': '不知道', '机构及科室': '心脏科', '联系人': 'winter', '联系方式': '13611012345','结果': '到位'}],
        '下次随访日期': [2024, 4, 1], '随访医生签名': '金浩1', '家长签名': "666"},

    '第3次': {'日期': [2024, 2, 2], '本次服务类别': {'随访': ''}, '随访方式': '电话', '孕周': ["1","3"],
        '产前检查机构名称': '1', '主诉': '2', '体重（kg）':'100','宫底高度(cm)':'144','腹围(cm)':'88','胎位':['右枕前（ROA）', '右枕后（LOP）'],'胎心率(次/分钟)':'45',
        '血压（mmHg）':['111','99'],'血红蛋白（g/L）':'234','尿蛋白': '++','其他辅助检查':'678',
        '随访结果分类': {'异常':'13123'},
        '指导': ['营养', '母乳喂养', {'其他': '4234'}],
        '转诊': ['有', {'原因': '不知道', '机构及科室': '心脏科', '联系人': 'winter', '联系方式': '13611012345','结果': '到位'}],
        '下次随访日期': [2024, 5, 1], '随访医生签名': '金浩1', '家长签名': "777"},

    '第4次': {'日期': [2024, 3, 3], '本次服务类别': {'随访': ''}, '随访方式': '电话', '孕周': ["1","3"],
        '产前检查机构名称': '1', '主诉': '2', '体重（kg）':'100','宫底高度(cm)':'144','腹围(cm)':'88','胎位':['右枕前（ROA）', '右枕后（LOP）'],'胎心率(次/分钟)':'45',
        '血压（mmHg）':['111','99'],'血红蛋白（g/L）':'234','尿蛋白': '++','其他辅助检查':'678',
        '随访结果分类': {'异常':'13123'},
        '指导': ['心理', '分娩准备', {'其他': '4234'}],
        '转诊': ['有', {'原因': '不知道', '机构及科室': '心脏科', '联系人': 'winter', '联系方式': '13611012345','结果': '到位'}],
        '下次随访日期': [2024, 6, 1], '随访医生签名': '金浩1', '家长签名': "888"},

    '第5次': {'日期': [2024, 4, 4], '本次服务类别': {'随访': ''}, '随访方式': '电话', '孕周': ["1", "3"],
            '产前检查机构名称': '1', '主诉': '2', '体重（kg）': '100', '宫底高度(cm)': '144', '腹围(cm)': '88',
            '胎位': ['右枕前（ROA）', '右枕后（LOP）'], '胎心率(次/分钟)': '45',
            '血压（mmHg）': ['111', '99'], '血红蛋白（g/L）': '234', '尿蛋白': '++', '其他辅助检查': '678',
            '随访结果分类': {'异常': '13123'},
            '指导': ['心理', '自我监测', {'其他': '4234'}],
            '转诊': ['有', {'原因': '不知道', '机构及科室': '心脏科', '联系人': 'winter', '联系方式': '13611012345', '结果': '到位'}],
            '下次随访日期': [2024, 7, 1], '随访医生签名': '金浩1', '家长签名': "999"},
}})

# 产后访视记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'operate': '编辑', 'title': '产后访视记录表', 'data': {}})

# 产后42天健康检查记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'operate': '编辑', 'title': '产后42天健康检查记录表', 'data': {}})
