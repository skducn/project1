...
# =========================================================================================================================================
    ...
    def queryReport(self, d_):
        # common - 统计报表之查询 - 外部调用
        signal.signal(signal.SIGINT, self.__handle_signal)
        signal.signal(signal.SIGTERM, self.__handle_signal)

        ele = Web_PO.getSuperEleByX("//section", ".")
        for k, v in d_.items():
            try:
                if k in ['管理机构', '所属机构']:
                    Web_PO.eleDropdown(Web_PO.eleCommon(ele, k), ".//div[2]/div/div/div/input",  v)
                elif k in ['时间']:
                    Web_PO.eleDropdownDate1(Web_PO.eleCommon(ele, k), ".//div[4]/div/div[1]/input", v[0])
                    Web_PO.eleDropdownDate1(Web_PO.eleCommon(ele, k), ".//div[4]/div/div[2]/input", v[1])
                elif k in [' 年度： ']:
                    Web_PO.eleSetTextEnterByX(Web_PO.eleCommon(ele, k), ".//div[4]/div/input", v)
                    # /html/body/div[1]/div/div[3]/section/div/div[1]/div[4]/div/input
            except:
                self.logger.error("查询 => " + str(k) + ": " + str(v))
