...
# Gw_PO.phs_maternalRecord_ycffiles_query({"管理机构": "招远市卫健局", '是否仅查询机构': '是', "档案状态": "暂不管理", "姓名": "胡成", "身份证号": "370685198705183027", '上次完成 检查类型': '第2次产前检查'})
...
# 第一次产前随访服务记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'title': '第一次产前随访服务记录表', 'data': {
...
# # 第2～5次产前随访服务记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'title': '第2～5次产前随访服务记录表', 'data': {
#     '第2次': {'日期': [2024, 1, 1], '本次服务类别': {'督促': ''}, '随访方式': '电话', '孕周': ["1", "3"],
...
#         '血压（mmHg）':['118', '100'],'血红蛋白（g/L）': '56','尿蛋白': '+++', '其他辅助检查': 'uu8', '随访结果分类': {'异常': 'ww13123'}, '指导': ['营养', '心理', {'其他': 'rt4234'}],
#         '转诊': {'有': {'原因': '56', '机构及科室': '外科', '联系人': 'wawa', '联系方式': '13312122334','结果': '未到位'}},
#         '下次随访日期': [2024, 5, 1], '随访医生签名': '1', '居民签名': "yoy12"},
#     '第4次': {'日期': [2024, 3, 3], '本次服务类别': {'失访': {'失访原因': '123'}},
#         '下次随访日期': [2024, 6, 1], '随访医生签名': '金浩1', '居民签名': "888"},
#     '第5次': {'日期': [2024, 4, 4], '本次服务类别': {'随访': ''}, '随访方式': '电话', '孕周': ["1", "3"],
#         '产前检查机构名称': '1', '主诉': '2', '体重（kg）': '100', '宫底高度(cm)': '144', '腹围(cm)': '88', '胎位': ['右枕前（ROA）'], '胎心率(次/分钟)': ['288'],
#         '血压（mmHg）': ['131', '89'], '血红蛋白（g/L）': '234', '尿蛋白': '++', '其他辅助检查': '678', '随访结果分类': {'异常': 'meme'}, '指导': ['心理', '自我监测', {'其他': 'hello'}],
#         '转诊': {'有': {'原因': '不知道', '机构及科室': '心脏科', '联系人': 'winter', '联系方式': '13611012345', '结果': '到位'}},
#         '下次随访日期': [2024, 7, 1], '随访医生签名': '金浩1', '居民签名': "999"},
# }})

# 产后访视记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'title': '产后访视记录表', 'data':
...
# 产后42天健康检查记录表
# Gw_PO.phs_maternalRecord_ycffiles_operation({'title': '产后42天健康检查记录表', 'data':
#     {'随访日期': [2024, 4, 4], '本次服务类别': {'失访': {'失访原因': '123'}}, '分娩日期': [2024, 2, 4], '出院日期': [2024, 5, 4],
