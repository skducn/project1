...
# 动态调用函数1
# s_func = 'phs_healthrecord_diagnosis_query'
# getattr(Gw_PO, s_func)({"身份证号": "41052220000511081"})
# 动态调用函数2(有风险)
# s_func = 'Gw_PO.phs_healthrecord_diagnosis_query({"身份证号": "41052220000511081"})'
# eval(s_func)
# *****************************************************************
from GwPO import *
...
varUrl = d_menu_basicPHS['接诊信息查询']
s_func = varUrl.split('http://192.168.0.203:30080/')[1]
s_func = s_func.replace("/", "_")
s_func = s_func.lower()
# print(s_func)  # phs_healthrecord_diagnosis
Web_PO.opnLabel(d_menu_basicPHS['接诊信息查询'])
Web_PO.swhLabel(1)


# 1 查询
# Gw_PO.phs_healthrecord_diagnosis_query()
getattr(Gw_PO, s_func + '_query')({"身份证号": "41052220000511081"})
# getattr(Gw_PO, s_func + '_query')({"姓名": "zhangsan", "身份证号": "410522200005110812", "就诊日期": [[2025, 1, 10], [2025, 2, 12]], "导入状态": "已导入", "管理机构": ["玲珑卫生院", "玲珑镇大蒋家村卫生室"]})
