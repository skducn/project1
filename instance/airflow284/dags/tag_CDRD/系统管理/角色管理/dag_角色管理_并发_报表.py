...
def generate_visualization_report(execution_results, report_path):
    """生成可视化测试报告"""
    try:
        print("📊 开始生成可视化测试报告...")
        ...
        fig.suptitle('测试执行报告', fontsize=16, fontweight='bold')

        # 1. 成功失败柱状图
        ax1 = plt.subplot(2, 3, 1)
        x_pos = range(len(modules))
        bars1 = ax1.bar(x_pos, success_counts, label='成功', color='green', alpha=0.7)
        bars2 = ax1.bar(x_pos, fail_counts, bottom=success_counts, label='失败', color='red', alpha=0.7)
        ax1.set_xlabel('测试模块')
        ax1.set_ylabel('数量')
        ax1.set_title('各模块执行结果')
        ...
        for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
            ...
            if height1 > 0:
                ax1.text(bar1.get_x() + bar1.get_width() / 2., height1 / 2.,
                         f'{int(height1)}', ha='center', va='center', color='white', fontweight='bold')
            if height2 > 0:
                ax1.text(bar2.get_x() + bar2.get_width() / 2., height1 + height2 / 2.,
                         f'{int(height2)}', ha='center', va='center', color='white', fontweight='bold')

        # 2. 成功率饼图
        ax2 = plt.subplot(2, 3, 2)
        successful_modules = sum(1 for rate in success_rates if rate == 100)
        failed_modules = len(modules) - successful_modules
        if failed_modules > 0:
            sizes = [successful_modules, failed_modules]
            labels = [f'全部成功 ({successful_modules})', f'部分失败 ({failed_modules})']
            colors = ['green', 'orange']
            ax2.pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
        else:
            ax2.pie([1], labels=['全部成功'], colors=['green'], autopct='%1.1f%%', startangle=90)
        ax2.set_title('模块成功率分布')

        # 3. 成功率条形图
        ax3 = plt.subplot(2, 3, 3)
        bars3 = ax3.bar(range(len(modules)), success_rates,
                        color=['green' if rate >= 90 else 'orange' if rate >= 70 else 'red' for rate in success_rates])
        ax3.set_xlabel('测试模块')
        ax3.set_ylabel('成功率 (%)')
        ax3.set_title('各模块成功率')
        ...
        # 这里可以添加实际的执行时间数据
        execution_times = [result.get('execution_time', 0) for result in execution_results.values()]
        if any(execution_times):
            ax4.plot(range(len(modules)), execution_times, marker='o', linewidth=2, markersize=6)
            ax4.set_xlabel('测试模块')
            ax4.set_ylabel('执行时间 (秒)')
            ax4.set_title('各模块执行时间')
            ax4.set_xticks(range(len(modules)))
            ax4.set_xticklabels(modules, rotation=45, ha='right')
            ax4.grid(True, alpha=0.3)
        else:
            ax4.text(0.5, 0.5, '暂无执行时间数据', transform=ax4.transAxes,
                     ha='center', va='center', fontsize=12)
            ax4.set_title('执行时间分析')

        # 5. 热力图 - 成功率矩阵
        ax5 = plt.subplot(2, 3, 5)
        if len(modules) > 1:
            # 创建相关性矩阵
            data_matrix = [[100 if i == j else success_rates[i] * success_rates[j] / 10000
                            for j in range(len(modules))] for i in range(len(modules))]
            sns.heatmap(data_matrix, annot=True, fmt='.1f', cmap='RdYlGn',
                        xticklabels=modules, yticklabels=modules, ax=ax5)
            ax5.set_title('模块成功率相关性')
        else:
            ax5.text(0.5, 0.5, '需要多个模块才能显示热力图', transform=ax5.transAxes,
                     ha='center', va='center', fontsize=10)
            ax5.set_title('模块相关性分析')

        # 6. 统计摘要
        ax6 = plt.subplot(2, 3, 6)
        ax6.axis('off')

        total_tests = sum(total_counts)
...
模块总数: {len(modules)}
完全成功模块: {sum(1 for rate in success_rates if rate == 100)}
部分失败模块: {sum(1 for rate in success_rates if 0 < rate < 100)}
完全失败模块: {sum(1 for rate in success_rates if rate == 0)}

📈 最高成功率: {max(success_rates):.1f}%
📉 最低成功率: {min(success_rates):.1f}%
📊 平均成功率: {sum(success_rates) / len(success_rates):.1f}%
        """
        ...
        print(f"❌ 生成可视化报告失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return False


def generate_html_report(execution_results, html_path):
    """生成HTML格式的详细测试报告"""
    try:
        print("📄 开始生成HTML测试报告...")
        ...
        # 统计汇总
        total_tests = sum(result.get('total_count', 0) for result in execution_results.values())
        total_success = sum(result.get('success_count', 0) for result in execution_results.values())
        total_fail = sum(result.get('fail_count', 0) for result in execution_results.values())
        overall_rate = (total_success / total_tests * 100) if total_tests > 0 else 0
...
<body>
    <div class="container">
        ...
        <div class="summary">
            ...
            <div class="metric">
                <div class="metric-value metric-success">{total_success}</div>
                <div>成功数</div>
            </div>
            <div class="metric">
                <div class="metric-value metric-fail">{total_fail}</div>
                <div>失败数</div>
            </div>
            <div class="metric">
                <div class="metric-value metric-rate">{overall_rate:.1f}%</div>
                <div>成功率</div>
            </div>
        </div>
        ...
        html_content += f"""
            ...
            <div class="chart-container">
                <h2>📈 执行概览</h2>
                <img src="test_report.png" alt="测试报告图表" style="max-width: 100%; height: auto;">
            </div>
        </div>

        <div class="footer">
            <p>© 2026 测试报告自动生成 | 执行引擎: Airflow</p>
        </div>
    </div>
</body>
</html>
...
def generate_test_report(**context):
    ...
    try:
        ...
        # 从XCom获取执行结果
        execution_results = context['task_instance'].xcom_pull(task_ids='并行执行消费者')

        if not execution_results:
            print("⚠️ 没有找到执行结果数据")
            return False

        print(f"📊 获取到 {len(execution_results)} 个模块的执行结果")

        # 生成报告文件路径
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_base_path = f"{web_project_root}/reports/test_report_{timestamp}"

        # 生成可视化图表报告
        chart_report_path = f"{report_base_path}.png"
        chart_success = generate_visualization_report(execution_results, chart_report_path)

        # 生成HTML详细报告
        html_report_path = f"{report_base_path}.html"
        html_success = generate_html_report(execution_results, html_report_path)

        # 生成JSON数据报告
        json_report_path = f"{report_base_path}.json"
        try:
            with open(json_report_path, 'w', encoding='utf-8') as f:
                json.dump(execution_results, f, ensure_ascii=False, indent=2)
            print(f"✅ JSON数据报告已生成: {json_report_path}")
        except Exception as e:
            print(f"❌ 生成JSON报告失败: {str(e)}")

        # 输出报告链接
        if chart_success or html_success:
            print("\n" + "=" * 60)
            print("🎉 测试报告生成完成!")
            print("=" * 60)
            if chart_success:
                print(f"📊 图表报告: {chart_report_path}")
            if html_success:
...
with DAG(
    ...
    # 设置依赖关系 - 真正的并行执行
    start >> producer >> parallel_consumers >> generate_report >> end
