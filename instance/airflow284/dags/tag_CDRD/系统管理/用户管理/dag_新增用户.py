...
# 重新计算项目根目录路径
# 从 /Users/linghuchong/Downloads/51/Python/project/instance/airflow284/dags/tag_CDRD/系统管理/用户管理
        ...
        if l_col_values:
            for i in range(len(l_col_values)):
                ...
                if l_col_values[i][1] == "系统管理" and l_col_values[i][2] == "用户管理" :
                    ...
                    print(f"找到匹配的测试用例: {l_col_values[i][5]}")
...
def final_task(**context):
    ...
    try:
        # 获取消费者的结果 - 修正task_id
        consumer1_result = context['task_instance'].xcom_pull(task_ids='新增用户')
