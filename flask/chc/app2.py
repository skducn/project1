...
def getRecordByResult(ruleName, result):
...
def list123(ruleName):
    ...
    if session_value == 'jinhao':
        ...
        if ruleName == '评估因素取值':
            ...
            return render_template('assessFactor.html', data=getFieldValue(ruleName), ruleName=ruleName, l_ruleSql=c, system=system)
        else:
...
def list4(ruleName, result):
    ...
    if session_value == 'jinhao':
        ...
        if ruleName == '评估因素取值':
            # return render_template('assessFactor2.html', data=getFieldValueByStep(ruleName), ruleName=ruleName, l_ruleSql=c, system=system, resultStatus=result)
            return render_template('assessFactor2.html', data=getRecordByResult(ruleName,result), ruleName=ruleName, l_ruleSql=c, system=system, resultStatus=result)
        else:
            return render_template('healthIntervention2.html', data=getRecordByResult(ruleName, result), ruleName=ruleName, l_ruleSql=c, system=system, resultStatus=result)
...
def get_queryResult():
    ...
    # if ruleName == '评估因素取值':
    #     return render_template('assessFactor.html', data=l_d_all, ruleName=ruleName, system=system)
    # else:
    #     print("1212")
    #     # return redirect('healthIntervention2.html')
    #     return render_template('healthIntervention2.html', data=l_d_all, ruleName=ruleName, system=system)
    print(l_d_all)
    # return l_d_all[0]
    response_data = {
        'text': str(l_d_all[0])
